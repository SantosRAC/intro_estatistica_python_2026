...
![Nível de Significância Alpha](dia_03_files/alpha.png)
...
data_groups = [penguins[penguins['species'] == s]['body_mass_g'] for s in species]
...
sns.stripplot(x='species', y='body_mass_g', data=penguins, color=".25", alpha=0.5, jitter=True)

# Função auxiliar para desenhar a barra de significância
def annotate_significance(x1, x2, y, p_value):
    h = 100  # altura da barra (bracket)
...
y_max = penguins['body_mass_g'].max() + 200
h_step = 400 
...
plt.ylim(penguins['body_mass_g'].min() - 200, y_max + h_step + 500)
plt.title('Massa Corporal: Comparações Estatísticas', fontsize=14)
plt.show()
```
-----
...
* **`scipy.stats`:** Distribuições: `.pdf`, `.cdf` e Testes: `ttest`, `shapiro`
* **`statsmodels`:** `ols`, ANOVA
* **`seaborn`:** Boxplot + Swarmplot
