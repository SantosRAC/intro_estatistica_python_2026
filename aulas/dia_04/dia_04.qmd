...
A análise de regressão permite examinar o efeito de uma **variável explicativa** (ou mais de uma) sobre uma **variável resposta**.
 ...
 - É possível fazer extrapolações a partir da variável explicativa
 
   - Tamanho de camiseta de criança
...
## Extrapolação
 ...
 - Dado um valor de X, predizer o valor de Y

Extrapolação indevida:

 - Valores muito fora do intervalo estudado

 - Previsão da economia na COVID-19
...
# clean rows needed for the model
df_clean = penguins_subset.dropna(subset=['body_mass_g', 'bill_depth_mm']).copy()

# features and target
X = df_clean[['body_mass_g']]
...
lin_reg.fit(X_train, # treinando o modelo
  y_train)
...
## Avaliando o modelo linear {.smaller}
 ...
 - Erro quadrático médio (EQM): quanto menor, melhor (de preferência menor que o desvio padrão)
...
## Avaliando o modelo linear {.smaller}
 ...
 - Coeficiente de determinação (R2): proporção da variação de Y explicada pela variação de X.
...
# performance
y_tr_pred = lin_reg.predict(X_train)
