...
## Agrupando os dados de pinguins (comprimento de bico e nadadeira) {.smaller}

:::: {.columns}

```{python}
penguins_subset_bill_flipper_info = penguins[['species', 'bill_length_mm', 'flipper_length_mm']]
print(penguins_subset_bill_flipper_info[['bill_length_mm', 'flipper_length_mm']])
```

::::

## Dados faltantes no dataset de pinguins (comprimento de bico e nadadeira) {.smaller}
...
## Escolha de k para comprimento de bico e nadadeira {.smaller}

 - Método do cotovelo com SSE (soma de quadrados intra-cluster, ou inércia)
...
```{python}
#| fig-align: center
X = penguins_subset_bill_flipper_info[['bill_length_mm', 'flipper_length_mm']].values
...
plt.xlabel('k')
plt.ylabel('SSE (Inércia)')
plt.title('Método do cotovelo vs k (bill_length_mm & flipper_length_mm)')
plt.grid(True)
plt.show()
```
...
plt.xlabel('k')
plt.ylabel('SSE (Inércia)')
plt.title('Método do cotovelo vs k (bill_length_mm & bill_depth_mm)')
plt.grid(True)
plt.show()
```
